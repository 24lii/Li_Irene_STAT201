...
cat("You generated", count, "numbers that sum to", num)
```
...
#running function
me <- bmi_calc(my_height, my_weight)
my_bmi <- me[1]
my_cat <- me[2]
cat("Your BMI is", my_cat, "with a value of", my_bmi)
...
```{r}
even_sum <- function(list){
  sum <- sum(list)
  even <- (sum %% 2 == 0)
  return(even)
}
output <- lapply(mylist, even_sum)
print(output)
```
...
fruits_red <- subset(fruit_info, fruit_info$color == "red")
print(fruits_red)
...
```{r}
weights <- fruit_info$weight
max_weight <- max(weights)
index <- which(weights == max_weight)

heavy_fruit <- fruit_info$fruit[index]
cat("heaviest fruit:", heavy_fruit)
...
#using weights from question above
weighed <- lapply(weights, is_light)
output <- ""
newline <- "                                                                                           "
for (i in 1:nrow(fruit_info)){
  output <- paste(output, "The", fruit_info$fruit[i], "is", weighed[i], newline)
}
print(output)
```
